...
function All() {
    ...
    const getData = async () => {
        const response = await axios.get(`${apiUrl}/admin/order/all`);
    ...
    const handleBtnConfirm = async (idOrder) => {
        const response = await axios.put(`${apiUrl}/admin/order/updateStatus/${idOrder}`, {status: 'Proccessing'});
    ...
    const handleBtnSuccess = async (idOrder) => {
        const response = await axios.put(`${apiUrl}/admin/order/updateStatus/${idOrder}`, {status: 'Successed'});
                ...
                order.map((el, index) => 
                    <div key={index} className={cx('wrapper-order')}>
                        ...
                        <div className={cx('wrapper-footer')}>
                            ...
                            {
                                el.status === 'Waiting' ? 
                                <button onClick={() => handleBtnConfirm(el._id)} className={cx('btn-cancel')}>Xác nhận đơn hàng</button> : 
                                el.status === 'Proccessing' ? 
                                <button onClick={() => handleBtnSuccess(el._id)} className={cx('btn-success')}>Đã giao hàng</button> : ""
                            }
                        </div>
